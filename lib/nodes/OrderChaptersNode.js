...
class OrderChaptersNode extends Node {
  ...
  getSchema() {
    return z.object({
      order: z.array(z.union([z.number(), z.string()]))
    });
  }
...
Return a JSON object with key 'order' containing an array of strings in the format "idx # AbstractionName".
`;
    ...
    // Response is already parsed JSON from callWithStructuredOutput
    let orderedIndicesRaw;

    // Handle the case where response might be a string or other unexpected format
    if (typeof response === 'string') {
      try {
        const parsed = JSON.parse(response);
        orderedIndicesRaw = parsed.order;
      } catch (e) {
        console.error('Failed to parse string response as JSON:', response);
        throw new Error("LLM returned invalid JSON string");
      }
    } else if (response && typeof response === 'object' && Array.isArray(response.order)) {
      orderedIndicesRaw = response.order;
    } else {
      throw new Error("LLM Output does not contain valid order array: " + typeof response);
    }
    ...
    for (const entry of orderedIndicesRaw) {
      ...
      if (typeof entry === 'number') {
        ...
        throw new Error(`Unexpected entry type in ordered list: ${typeof entry}`);
      }
