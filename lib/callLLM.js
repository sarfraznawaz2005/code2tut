...
class LLMCaller {
  ...
  /**
   * Call the LLM with structured output using YAML parsing
   * @param {string} prompt - The prompt to send to the LLM
   * @param {boolean} shouldCache - Whether to cache this response
   * @returns {Object} - The parsed YAML response
   */
  async callWithStructuredOutput(prompt, shouldCache = true) {
    // Check cache first if enabled
    if (this.useCache && shouldCache) {
      const cachedResponse = this.getCachedResponse(prompt);
      if (cachedResponse !== null) {
    ...
    for (let attempt = 0; attempt < this.retry.attempts; attempt++) {
      try {
        ...
        let parsedResponse;
        try {
          // Extract YAML from the response content
          const content = response.content;
          let yamlStr = content;

          // Look for YAML between ```yaml and ``` markers
          let yamlMatch = content.match(/```yaml\s*([\s\S]*?)\s*```/);
          if (yamlMatch) {
            yamlStr = yamlMatch[1];
          } else {
          ...
          // Clean up the YAML string if needed
          yamlStr = yamlStr.trim();
          parsedResponse = yaml.load(yamlStr);
        } catch (parseError) {
          console.error('Failed to parse response as YAML:', parseError);
